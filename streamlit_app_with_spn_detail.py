...
def make_result_excel(original_base_df, original_mpn_df, mapped_df, compare_df, missing_spn_df, summary_df):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        missing_spn_fill = PatternFill(fill_type="solid", start_color="C00000", end_color="C00000")
        ambiguous_fill = PatternFill(fill_type="solid", start_color="F4CCCC", end_color="F4CCCC")
        missing_in_system_fill = PatternFill(fill_type="solid", start_color="FCE5CD", end_color="FCE5CD")
        extra_in_system_fill = PatternFill(fill_type="solid", start_color="F9CB9C", end_color="F9CB9C")
        partial_fill = PatternFill(fill_type="solid", start_color="FFF2CC", end_color="FFF2CC")

        headers = [cell.value for cell in ws[1]]
        if "MPN_Compare_Status" in headers:
            ...
            for row_idx in range(2, ws.max_row + 1):
                ...
                if status == "Missing SPN":
                    ...
                    fill = missing_in_system_fill
                elif status == "Extra in System":
                    fill = extra_in_system_fill
...
娱乐平台主管 to=python_user_visible.exec code
from pathlib import Path
p = Path("/mnt/data/streamlit_app_with_spn_detail_v2.py")
text = p.read_text(encoding="utf-8")
text = text.replace('st.error(f"Processing failed: {e}\')', 'st.error(f"Processing failed: {e}")')
p.write_text(text, encoding="utf-8")
print("Fixed syntax and saved:", p)
